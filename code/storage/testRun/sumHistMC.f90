...
!
! It will read histogramMC files from several directories, indicated by 'dirName' and a number,
! and perform a (weighted) summation/average of all the histograms.
!
! Typcial application: Summing histogram files from several heavy-ion runs with different impact parameters.

program sum_HistMC

  use constants, only: pi
  use histMC, only: histogramMC, ReadHistMC, WriteHistMC, sumHistMC

  implicit none

  type(histogramMC) :: sum_hist, tmp_hist
  ...
  do j=1,NumFiles
    ...
    do i0=0,nDirs
      ...
      ! read histogram file
      if (ReadHistMC (sum_hist, f, w)) then
         s = w
    ...
    do i=i0+1,nDirs
      ...
      ! read histogram file
      if (ReadHistMC (tmp_hist, f)) then
         s = s + w
         ! add histogram
         call sumHistMC(sum_hist, tmp_hist, w)
         write(*,'(A)',advance="NO") "."
      else
    ...
    call WriteHistMC (sum_hist, file=trim(fileNames(j)), mul=xs/s)
