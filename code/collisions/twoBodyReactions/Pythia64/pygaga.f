c===================================================================
c This is a modified version of original PYTHIA-PYGAGA (v6.419)
c
c * x and Q2 are picked at lower bound, not randomly choosen 
c   in the interval
c * instead of random phi-selection for the photon, phi is 
c   fixed to be 0.
c
C*********************************************************************
...
!            Q2MAX(I)=XMAX(I)**2*PMS(I)/(1D0-XMAX(I))+           ! [GiBUU]
!     &      ((1D0-XMIN(I))*(VINT(302)-2D0*PMS(3-I))-            ! [GiBUU]
!     &      2D0*PMS(I)/(1D0-XMIN(I)))*SIN(THEMAX/2D0)**2        ! [GiBUU]
!            IF(CKIN(64+2*I).GT.0D0) Q2MAX(I)=MIN(CKIN(64+2*I),Q2MAX(I))
            Q2MAX(I)=CKIN(64+2*I)                                ! [GiBUU]
  ...
  120   LOOP=LOOP+1
        ...
        if (LOOP.eq.10000) then
           ...
           do i=1,2
              IF(MINT(140+I).NE.0) THEN
                 ...
                 write(*,*) 'X:    ',X(I)
                 write(*,*) 'Q2:   ',Q2(I)
                 write(*,*) '...:  ',X(I)**2*PMS(I)/(1D0-X(I))
                 write(*,*) '---:  ',
     $                (Q2(I).LT.X(I)**2*PMS(I)/(1D0-X(I)))
                 write(*,*) 'Y:    ',Y(I)
                 write(*,*) '---:  ',(Y(I).LT.CKIN(71+2*I)),
     $                (Y(I).GT.CKIN(72+2*I))

                 write(*,*) '...:  ',CKIN(71+2*I),CKIN(72+2*I)
                 write(*,*) 'PMC:  ',PMC(I),PMC(3)
     ...
     &             ((1D0-X(I))**2*(VINT(302)-2D0*PMS(3-I)-2D0*PMS(I)))
        ...
        DO 130 I=1,2
          IF(MINT(140+I).NE.0) THEN
C...Pick x and Q2
!             X(I)=XMIN(I)*(XMAX(I)/XMIN(I))**PYR(0)     ! [GiBUU]
!            Q2(I)=Q2MIN(I)*(Q2MAX(I)/Q2MIN(I))**PYR(0)  ! [GiBUU]
             X(I)=XMIN(I)                                ! [GiBUU]
             Q2(I)=Q2MIN(I)                              ! [GiBUU]
C...Cuts on internal consistency in x and Q2.
            IF(Q2(I).LT.X(I)**2*PMS(I)/(1D0-X(I))) GOTO 120
!            IF(Q2(I).GT.(1D0-X(I))*(VINT(302)-2D0*PMS(3-I))- ! [GiBUU]
!     &      (2D0-X(I)**2)*PMS(I)/(1D0-X(I))) GOTO 120        ! [GiBUU]
