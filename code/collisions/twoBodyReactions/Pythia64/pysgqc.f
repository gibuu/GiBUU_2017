c===================================================================
c This is a modified version of original PYTHIA-PYSGQC (v6.419)
c
c * longitudinal contrib for DIS
C*********************************************************************
...
C...f + gamma* -> f.
     ...
     &        Q2GA**0.57D0)
              ...
              XY=PARP(169-ISDE)                            ! [GiBUU]
              IF(MSTP(16).EQ.0) THEN                       ! [GiBUU]
                 IF(VINT(307-ISDE).GT.0D0.AND.VINT(307-ISDE).LT.1D0)
     &                XY=VINT(307-ISDE)                    ! [GiBUU]
              ELSE                                         ! [GiBUU]
                 IF(VINT(311-ISDE).GT.0D0.AND.VINT(311-ISDE).LT.1D0)
     &                XY=VINT(311-ISDE)                    ! [GiBUU]
              ENDIF                                        ! [GiBUU]
              BEAMAS=PYMASS(11)                            ! [GiBUU]
              IF(VINT(305-ISDE).GT.0D0) BEAMAS=VINT(305-ISDE)
              if (XY.gt.0D0) then                          ! [GiBUU]
                 FRACLT=1D0/(1D0+(XY**2*(1D0-2D0*BEAMAS**2/Q2GA))/
     &                (2D0/(1D0+Q2GA/XY**2/VINT(290)**2)*(1D0-XY-
     &                (Q2GA/4D0/VINT(290)**2))))           ! [GiBUU]
              ELSE                                         ! [GiBUU]
                 FRACLT=0D0                                ! [GiBUU]
              ENDIF                                        ! [GiBUU]
!              r2GA=VINT(2+ISDE)**2                         ! [GiBUU]
!              COMFAC=COMFAC*(1D0+FRACLT*Q2GA*4*r2GA/(W2GA+Q2GA-r2GA)**2)
              if (Q2GA.gt.0.35) then                       ! [GiBUU]
                 r2GA = 0.0635*(1+12*Q2GA*0.125**2/((Q2GA+1)*
     $                (0.125**2+XGA**2)))/log(Q2GA/0.04)   ! [GiBUU]
     $                + 0.5747/Q2GA - 0.3534/(Q2GA**2+0.09)! [GiBUU]
              else                                         ! [GiBUU]
                 r2GA = 0.0635*(1+12*0.35*0.125**2/((0.35+1)*
     $                (0.125**2+XGA**2)))/log(0.35/0.04)   ! [GiBUU]
     $                + 0.5747/0.35 - 0.3534/(0.35**2+0.09)! [GiBUU]
              endif                                        ! [GiBUU]
              COMFAC=COMFAC*(1D0+FRACLT*r2GA)              ! [GiBUU]
